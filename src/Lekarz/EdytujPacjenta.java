...
public class EdytujPacjenta extends JFrame {
	...
	public EdytujPacjenta() {
		...
		JLabel lblChoroba = new JLabel("Choroba:");
		lblChoroba.setBounds(92, 124, 61, 16);
		contentPane.add(lblChoroba);
		
		JLabel lblSala = new JLabel("Sala:");
		lblSala.setBounds(92, 152, 61, 16);
		contentPane.add(lblSala);
		
		sala = new JTextField();
		sala.setBounds(200, 147, 130, 26);
		contentPane.add(sala);
		sala.setColumns(10);
		
		choroba = new JTextField();
		choroba.setBounds(200, 119, 130, 26);
		contentPane.add(choroba);
		choroba.setColumns(10);
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String query = "SELECT Sala,Choroba FROM Pacjent WHERE Imie = '" + im + "' AND Nazwisko = '" + nazw + "'";
		...
		btnNewButton_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String chor = choroba.getText();
				
				String query = "UPDATE Pacjent SET Choroba = '" + chor + "', Sala = '" + sala1 + "' WHERE Imie = '" + im + "' AND Nazwisko = '" + nazw + "'";
				System.out.println(query);
				
				Mquery.main();
				Mquery.update(query);
				
				JOptionPane.showMessageDialog(contentPane, "Pacjent edytowany");
				setVisible(false);
		...
		btnWr.setBounds(437, 327, 117, 29);
